...
#         self.smoothness_loss_weight = 0.005
        ...
        self.contrastive_loss_weight = 0.3 #.3 #0.2
...
#                      }
    ...
    def get_outer_loss(self, batch, save_pth=None):
        ...
        train_inputs_perdomin, train_targets_perdomin, meta_train_hs_masks, label_his_b, label_his_f = batch['train']
        ...
        train_hs_perdomain = rearrange(meta_train_hs_masks, 'b d db c h w -> d (b db) c h w')
        ...
        for task_id, (train_inputs, train_targets, train_hs, test_inputs, test_targets, test_hs, domain_name) \
...
#             print('!!!!!!!!!!!!!!!!!!!!!!!!domain name', domain_name)
            ...
            params, adaptation_results = self.adapt(train_inputs, train_targets, train_hs, label_his_b, label_his_f, domain_name, 
            ...
            with torch.set_grad_enabled(self.model.training):
                ...
                test_logits, _, layer_features, hist_features = self.model(test_inputs, params=params) # for ours
                # test_logits, compact, embeddings = self.model(test_inputs, params=params)
...
#                 embeddings = self.model_embedding(layer_features)
...
#                 print('C2R LOSS:', contrastive_loss)
    ...
    def adapt(self, inputs, targets, hs, targets_his_b, targets_his_f, domain_name, is_classification_task=None,
...
#         print('targets_his_f shape:', targets_his_f.shape)
        ...
        for step in range(num_adaptation_steps):
            logits, _, layer_features, hist_features= self.model(inputs, params=params)
...
#                 print('loss_hist_b each iter:', loss_hist_f)
            ...
            # ###################################################################
            embeddings = self.model_embedding(layer_features)
            train_ps = (hs[0]).reshape(hs[0].shape[0], 1, hs[0].shape[1], hs[0].shape[2])
            train_hs_n1 = (hs[1]).reshape(hs[1].shape[0], 1, hs[1].shape[1], hs[1].shape[2])
            train_hs_n2 = (hs[2]).reshape(hs[2].shape[0], 1, hs[2].shape[1], hs[2].shape[2])
            train_hs_n3 = (hs[3]).reshape(hs[3].shape[0], 1, hs[3].shape[1], hs[3].shape[2])
            
            contrastive_loss = self.contrastive_loss(embeddings, logits, targets, train_ps, \
                                                         train_hs_n1, train_hs_n2, train_hs_n3, self.cl_hs_weights[domain_name[0]])
...
#             print('contrastive loss:', contrastive_loss)
            ...
            inner_loss = bce_loss + hist_loss #+ contrastive_loss
        ...
        while num_batches < max_batches:
            for batch in dataloader:
                ...
                with torch.no_grad():
                    ...
                    seg_result, output, _,_ = self.model(inputs)
                ...
                output.save((save_pth_img +'/em_feature_{}.png').format(num_batches))
